...
def _is_tool_node_match(node: ast.AST, tool_name: str, base_name: str = 'tools') -> bool:
    """
    Check if an AST node matches a tool reference.
...
def _process_tools_list(
    ...
    if operation == 'add':
        ...
        # Add tools.* if bundled
        if tool_data.get("tools_bundled"):
            new_tools.insert(0, _create_tool_attribute('*', base_name))

        # Add new tools
        new_tools.extend(_create_tool_attributes(tool_data["tools"], base_name))
        return new_tools
