...
class JobpostingCrew:
    ...
    def review_agent(self) -> Agent:
        return Agent(config=self.agents_config["review_agent"], tools=[], verbose=True)

    @agent
    def writer_agent(self) -> Agent:
        return Agent(config=self.agents_config["writer_agent"], tools=[], verbose=True)

    @agent
    def researcher_agent(self) -> Agent:
        return Agent(
            config=self.agents_config["research_agent"],
            tools=[tools.web_scrape, tools.web_crawl, tools.retrieve_web_crawl],
            verbose=True,
        )
    ...
    def review_and_edit_job_posting_task(self) -> Task:
        return Task(config=self.tasks_config["review_and_edit_job_posting_task"])
