...
class CLIToolsTest(unittest.TestCase):
    ...
    def test_add_tool(self, tool_name):
        ...
        result = run_cli(CLI_ENTRY, 'init', f"{tool_name}_project")
        self.assertEqual(result.returncode, 0)
        os.chdir(self.project_dir / f"{tool_name}_project")
        result = run_cli(CLI_ENTRY, 'generate', 'agent', 'test_agent', '--llm', 'opeenai/gpt-4o')
        self.assertEqual(result.returncode, 0)
        result = run_cli(CLI_ENTRY, 'generate', 'task', 'test_task')
        self.assertEqual(result.returncode, 0)

        result = run_cli(CLI_ENTRY, 'tools', 'add', tool_name)
        print(result.stdout)
        self.assertEqual(result.returncode, 0)
