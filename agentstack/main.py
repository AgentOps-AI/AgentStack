...
def main():
    ...
    init_parser = subparsers.add_parser(
        "init", aliases=["i"], help="Initialize a directory for the project"
    )
    init_parser.add_argument(
        "slug_name", nargs="?", help="The directory name to place the project in"
    )
    init_parser.add_argument(
        "--wizard", "-w", action="store_true", help="Use the setup wizard"
    )
    ...
    generate_parser = subparsers.add_parser(
        "generate", aliases=["g"], help="Generate agents or tasks"
    )
    ...
    agent_parser = generate_subparsers.add_parser(
        "agent", aliases=["a"], help="Generate an agent"
    )
    ...
    task_parser = generate_subparsers.add_parser(
        "task", aliases=["t"], help="Generate a task"
    )
    ...
    task_parser.add_argument(
        "--expected_output", "-e", help="Expected output of the task"
    )
    ...
    tools_subparsers = tools_parser.add_subparsers(
        dest="tools_command", help="Tools commands"
    )
    ...
    tools_add_parser = tools_subparsers.add_parser(
        "add", aliases=["a"], help="Add a new tool"
    )
    ...
    tools_remove_parser = tools_subparsers.add_parser(
        "remove", aliases=["r"], help="Remove a tool"
    )
    ...
    if args.command in ["docs"]:
        ...
        if args.generate_command in ["agent", "a"]:
            ...
            generation.generate_agent(
                args.name, args.role, args.goal, args.backstory, args.llm
            )
